export const gradients = {
  // Primary brand gradient (hero text, CTAs)
  brand:
    "linear-gradient(90deg, #3B6FE2 0%, #6D28D9 100%)",

  // Soft brand background glow
  brandSoft:
    "linear-gradient(180deg, rgba(59,111,226,0.15) 0%, rgba(147,51,234,0.15) 100%)",

  // Hero canvas / large background glow
  heroGlow:
    "radial-gradient(circle at center, rgba(59,111,226,0.25) 0%, transparent 70%)",

  // Section highlight gradient
  section:
    "linear-gradient(135deg, #1E40AF 0%, #3B6FE2 50%, #6D28D9 100%)",

  // CTA dark overlay (footer / big CTAs)
  ctaDark:
    "linear-gradient(180deg, rgba(30,64,175,0.85) 0%, rgba(109,40,217,0.85) 100%)",

  // Subtle divider gradient
  divider:
    "linear-gradient(90deg, transparent 0%, rgba(59,111,226,0.4) 50%, transparent 100%)",
};
